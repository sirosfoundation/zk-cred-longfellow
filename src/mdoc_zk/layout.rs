...
/// Determines the layout of the signature circuit and hash circuit inputs for the mdoc_zk system.
...
impl InputLayout {
    ...
    pub(super) fn hash_input_length(&self) -> usize {
        ...
        self.hash_statement_length()
            ...
            + sha_256_input_wires(sha_256_max_blocks)
            + sha_256_witness_wires(sha_256_max_blocks)
            + 12 // validFrom
            + 12 // validUntil
            + 12 // deviceKeyInfo
            + 12 // valueDigests
...
// ── Signature circuit structs (unchanged) ─────────────────────────────────
...
impl<'a> EcdsaWitness<'a> {
    const LENGTH: usize = 5 + 8 + 256 + 255 * 3;
...
impl<'a> AttributeInputV6<'a> {
    const LENGTH: usize = ATTRIBUTE_CBOR_DATA_LENGTH_V6 * 8 + 8;
}
